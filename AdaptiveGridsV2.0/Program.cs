...
IFiniteElement[] elements = { new TriangleFEQuadraticBaseWithNI("volume", new int[] { 0, 1, 2 }), new TriangleFEQuadraticBaseWithNI("volume", new int[] { 1, 2, 4 }),
                              new TriangleFEQuadraticBaseWithNI("volume", new int[] { 2, 3, 4 }), new TriangleFEQuadraticBaseWithNI("volume", new int[] { 0, 2, 3 }),
                              new TriangleFEStraightQuadraticBaseWithNI("1", new int[] { 0, 1 }), new TriangleFEStraightQuadraticBaseWithNI("2", new int[] { 1, 4 }),
                              new TriangleFEStraightQuadraticBaseWithNI("3", new int[] { 3, 4 }), new TriangleFEStraightQuadraticBaseWithNI("4", new int[] { 0, 3 })};
...
FiniteElementMesh mesh = new FiniteElementMesh(elements, vertex);
...
materials.Add("volume", new Material(true, false, false, x => x.X * x.X, x => 4, (x, t) => 0, (x, t) => 0, (x, t) => -2 * x.X + 4));
materials.Add("1", new Material(false, false, true, x => 2, x => 4, (x, t) => -x.X * x.X, (x, t) => x.X + t, (x, t) => 4));
...
while (flag != "no")
{
   ...
   Console.WriteLine($"Значение в точке численного решения ({x};{y}) = " + solution.Value(point));
   Console.WriteLine($"Значение в точке реального решения ({x};{y}) = " + RealFunc(point, solution.Time));
   ...
   foreach (var element in mesh.Elements)
   {
      if (element.VertexNumber.Length != 2)
      {
         if (element.IsPointOnElement(mesh.Vertex, point))
         {
            ...
            Vector2D gradReal = RealGradientFunc(point);
